...
namespace LTNet
{
		...
		public Message PopMessage()
		{
			...
			if (this.Count >= 4)
			{
				...
				if (packLen > 0)
				{
					...
					DebugUtil.LogWarning("[DataHolder Pop] package split, waiting for complete data");
					return null;
				}

				DebugUtil.LogError("[DataHolder Pop] wrong package length");
				return null;
			}

			DebugUtil.LogError("[DataHolder Pop] cannot read package length");
			return null;
		}
	...
	public class SocketConnector : MonoBehaviour, ISocketConnector
	{
		...
		public void Connect()
		{
			DebugUtil.Log("[SocketConnector connect] with " + (mUsingAsync ? "Async mode" : "Sync mode"));
			...
			{
				DebugUtil.LogError("[SocketConnector connect] exception: " + e.Message);
				return;
			}
