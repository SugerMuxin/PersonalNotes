...
namespace LTNet
{
	...
	public class NetSocket : INetSocket
	{
		...
		public virtual void Connect()
		{
			...
			{
				DebugUtil.LogError("[NetSocket Connect] exception: " + e.Message);
			}
		}
		...
		protected void InternalAsyncDone(IAsyncResult asyncResult)
		{
			...
			try
			{
				...
				if (state.mTcpClient != null && state.mTcpClient.Client != null)
				{
					...
					DebugUtil.Log("[NetSocket InternalAsyncDone]");
				}
				else
				{
					DebugUtil.Log("[NetSocket InternalAsyncDone] connection is probably closed");
					return;
				}
			}
			catch (Exception e)
			{
				DebugUtil.LogError("[NetSocket InternalAsyncDone] exception: " + e.Message);
			}
		...
		public virtual void ConnectAsync(Action<object>callback, object userdata)
		{
			...
			{
				DebugUtil.LogError("[NetSocket ConnectAsync] exception: " + e.Message);
			}
		}
		...
		public virtual void Disconnect()
		{
			...
			{
				DebugUtil.Log("[NetSocket Disconnect]");
		...
		public virtual bool Send(byte[] data)
		{
			try
			{
				...
				{
					DebugUtil.LogError("[NetSocket Send] NetStream cannot write");
					return false;
				}

				if(data == null || data.Length <= 0)
				{
					DebugUtil.LogError ("[NetSocket Send] data not valid");
					return false;
				}
			...
			{
				DebugUtil.LogError("[NetSocket Send] exception: " + e.Message);
				return false;
			}
		}
		...
		public virtual int Receive(byte[] buffer)
		{
			...
			{
				DebugUtil.LogError("[NetSocket Receive] exception: " + e.Message);
				return -1;
			}
		}
